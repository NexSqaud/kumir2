...
namespace LLVMCodeGenerator {
    ...
    void LLVMGenerator::addFunction(const AST::AlgorithmPtr kfunc, bool createBody)
    {
        ...
        if (createBody) {
            ...
            for (; lIt != lfn->args().end() && kIt != namedArgs.end(); ++lIt, ++kIt)
            {
                llvm::Argument& larg = *lIt;
    ...
    llvm::Value* LLVMGenerator::findVariableAtCurrentContext(const AST::VariablePtr& kvar)
    {
        ...
        if (isRetval) {
            llvm::Argument* firstArg = currentFunction_.rawPtr()->getArg(0);
            var = firstArg;
        }
        else {
    ...
    llvm::Value* LLVMGenerator::calculate(llvm::IRBuilder<>& builder, const AST::ExpressionPtr& ex, bool isLvalue)
    {
        ...
        else if (ex->kind == AST::ExprConst && ex->baseType.kind == AST::TypeUser)
        {
            QVariant value = ex->constant;

            if (value.type() == QMetaType::QVariantList)
            {
                QVariantList list = ex->constant.toList();
                result = createRecord(builder, list.size(), ex->keepInCache ? FunctionBegin : AsIs);
                for (int i = 0; i < list.size(); i++)
                {
                    addRecordField(builder, result, i, list[i]);
                }
            }
            else
            {
                qFatal("User type not a QVariantList (%d, %s)", value.userType(), value.typeName());
            }
    ...
    llvm::Value* LLVMGenerator::createRecord(llvm::IRBuilder<>& builder, size_t fieldsCount, AllocaPlace allocaPlace)
    {
        llvm::Value* result = nullptr;
        LLVM::TypeRef type = getScalarType();
        Q_ASSERT(type);
        llvm::Value* fieldsCountValue = llvm::ConstantInt::getSigned(llvm::Type::getInt32Ty(*context_), fieldsCount);
        llvm::Value* tmp = CreateAlloca(builder, type, "", FunctionBegin);
        result = kumirCreateRecord_.createCallInstruction(&builder, tmp, fieldsCountValue);
        Q_ASSERT(result);
        return tmp;
    }

    void LLVMGenerator::addRecordField(llvm::IRBuilder<>& builder, llvm::Value* record, size_t fieldIndex, QVariant& value)
    {
        LLVM::TypeRef type = getScalarType();
        llvm::Value* arg = nullptr;
        LLVM::FunctionRef func;
        Q_ASSERT(type);
        llvm::Value* fieldIndexValue = llvm::ConstantInt::getSigned(llvm::Type::getInt32Ty(*context_), fieldIndex);
        ...
        {
            qFatal("Unsupported type (%d, %s)", value.type(), value.typeName());
        }
        llvm::Value* tmp = CreateAlloca(builder, type, "", FunctionBegin);
        llvm::Value* tmpAlloc = func.createCallInstruction(&builder, tmp, arg);
        Q_ASSERT(tmpAlloc);
        llvm::Value* result = kumirAddRecordField_.createCallInstruction(&builder, record, fieldIndexValue, tmp);
        Q_ASSERT(result);
    }
    ...
    void LLVMGenerator::readStdLibFunctions()
    {
        ...
        kumirCreateRecord_ = stdlibModule_.getFunction("__kumir_create_record");
        Q_ASSERT(kumirCreateRecord_);

        kumirAddRecordField_ = stdlibModule_.getFunction("__kumir_add_record_field");
        Q_ASSERT(kumirAddRecordField_);
