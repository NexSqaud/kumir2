...
public:
    inline explicit Application(int &argc, char **argv, bool gui)
        : QObject(), _qApp(0), _timerId(-1), _splashScreen(nullptr), _started(false)
    {
        if (gui)
        {
...
#error No default configuration passed to GCC
        ...
        if (!error.isEmpty())
        {
            ...
            _qApp->exit(_qApp->property("returnCode").isValid()
                            ? _qApp->property("returnCode").toInt()
                            : 1);
        }
...
bool setup_custom_vendor_information(QCoreApplication *app)
{
    QString appName;
    QString appVendor;
    QString appVersion;
